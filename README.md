...
После этого я практически перекроил проэкт и чесно говоря забил на документацию и на изначальную идею того как должен это сделать.

Поэтому код что есть.. берите и используйте.

А вот ссылка на пока рабочий сайт: http://kalashnikovsystem.com/ (скорее всего уже не работает так как мне понадобился домен).

Длятого чтобы запустить этот код у себя вам нужно что-то сделать с почтовым клиентом так как я использовал Mailgun и тут все под него заточено просто вставте логин пароль в файл переменных и почту с которой будете слать.
